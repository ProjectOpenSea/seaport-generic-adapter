...
contract GenericMarketplaceTest is BaseMarketplaceTest {
    ...
    function benchmark_BuyOfferedERC721WithEther_ListOnChain(
        ...
        try config.getPayload_BuyOfferedERC721WithEther(
            TestOrderContext(true, true, alice, bob), // TODO: come back and add a test for the wrapped version specifically
        ...
        prepareTest(config)
    {
        ...
        test721_1.mint(alice, 1);
        try config.getPayload_BuyOfferedERC721WithEther(
            TestOrderContext(false, false, alice, bob),
            TestItem721(address(test721_1), 1),
            100
        ) returns (TestOrderPayload memory payload) {
    ...
    function benchmark_BuyOfferedERC1155WithEther_ListOnChain(
        ...
        try config.getPayload_BuyOfferedERC1155WithEther(
            TestOrderContext(true, false, alice, bob),
    ...
    function benchmark_BuyOfferedERC721WithEtherFee_ListOnChain(
        ...
        try config.getPayload_BuyOfferedERC721WithEtherOneFeeRecipient(
            TestOrderContext(true, true, alice, bob),
            TestItem721(address(test721_1), 1),
        ...
        prepareTest(config)
    {
        ...
        test721_1.mint(alice, 1);
        try config.getPayload_BuyOfferedERC721WithEtherOneFeeRecipient(
            TestOrderContext(false, true, alice, bob),
            TestItem721(address(test721_1), 1),
            100,
            feeReciever1,
            5
    ...
    function benchmark_BuyTenOfferedERC721WithEther_ListOnChain(
        ...
        try config.getPayload_BuyOfferedManyERC721WithEther(
            TestOrderContext(true, false, alice, bob), nfts, 100
        ) returns (TestOrderPayload memory payload) {
            _benchmarkCallWithParams(
                config.name(),
                string(abi.encodePacked(testLabel, " List")),
        ...
        prepareTest(config)
    {
        ...
        try config.getPayload_BuyOfferedManyERC721WithEther(
            TestOrderContext(false, false, alice, bob), nfts, 100
        ) returns (TestOrderPayload memory payload) {
            for (uint256 i = 0; i < 10; i++) {
    ...
    function benchmark_BuyTenOfferedERC721WithEtherDistinctOrders(
        ...
        for (uint256 i = 0; i < 10; i++) {
            test721_1.mint(alice, i + 1);
            nfts[i] = TestItem721(address(test721_1), i + 1);
            contexts[i] = TestOrderContext(false, false, alice, bob);
            ethAmounts[i] = 100 + i;
        }
    ...
    function benchmark_BuyTenOfferedERC721WithEtherDistinctOrders_ListOnChain(
        ...
        for (uint256 i = 0; i < 10; i++) {
            test721_1.mint(alice, i + 1);
            nfts[i] = TestItem721(address(test721_1), i + 1);
            contexts[i] = TestOrderContext(true, false, alice, bob);
            ethAmounts[i] = 100 + i;
        }
        ...
        prepareTest(config)
    {
        ...
        contexts[0] = TestOrderContext(false, false, alice, address(0));
        contexts[1] = TestOrderContext(false, false, cal, address(0));
        contexts[2] = TestOrderContext(false, false, bob, address(0));
