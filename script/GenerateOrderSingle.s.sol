...
contract GenericMarketplaceAggregationScript is Script, GenerateOrderGeneric {
    ...
    // function fulfillMixedOrdersIndividuallyThroughAdapter() external {
    //     BaseMarketConfig[] memory configs = new BaseMarketConfig[](4);
    //     configs[0] = foundationConfig;
    //     configs[1] = looksRareV2Config;
    //     configs[2] = seaportOnePointFiveConfig;
    //     configs[3] = sudoswapConfig;
    //     configs[4] = x2y2Config;
    //     configs[5] = zeroExConfig;
    ...
    // buyOfferedERC721WithWETH_Adapter(configs[2]);
    ...
    //     infra.adapterConsiderationArray[0] =
    ...
    //     // Stick the items into the adapter so that seaport can yank them
    // out.
    ...
    //     // This should provide all the info required for the aggregated
    // orders.
    ...
    //     assertEq(weth.balanceOf(alice), 100, "Alice did not get the
    // wrapped");

    //     assertEq(test20.balanceOf(alice), 100, "Alice did not get the
    // test20");
    ...
    //     assertEq(test721_1.ownerOf(1), alice, "Alice does not have the 721
    // 1");
    //     assertEq(test721_1.ownerOf(2), alice, "Alice does not have the 721
    // 2");
    //     assertEq(test721_1.ownerOf(3), alice, "Alice does not have the 721
    // 3");
    //     assertEq(test721_1.ownerOf(4), alice, "Alice does not have the 721
    // 4");
    ...
    //         string(
    //             abi.encodePacked(
    //                 infra.testLabel, " Fulfill aggregated w/
    // fulfillAvailable"
    //             )
    //         ),
    //         true,
    ...
    //     assertEq(test20.balanceOf(alice), 400, "Alice did not get the
    // test20");
    ...
    //             "BaseMarketplaceTester::prepareAggregationTest: approval
    // target not set"
    //         );
    //         _setApprovals(
    //             alice,
    ...
    // {
    //     try
    // infra.configs[0].getPayload_BuyOfferedERC721WithEtherOneFeeRecipient(
    //         infra.context,
    //         standardERC721,
    ...
    //     infra.context = OrderContext(
    //         false, true, stdCastOfCharacters // should be real values lol
    //     );
    ...
    //     // Start with all buy offered NFT with X. But eventually test a mix
    // of
    ...
    //             : basicOrderType > 7
    //                 ? ItemType.ERC721
    //                 : basicOrderType > 3 ? ItemType.ERC1155 :
    // ItemType.ERC721;
