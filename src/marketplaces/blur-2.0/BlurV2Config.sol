    ...
    Merkle
{
    ...
    function beforeAllPrepareMarketplaceCall(
        ...
        setupCalls[0] = SetupCall(
            ...
            abi.encodeWithSelector(
                IBlurExchangeV2.setOracle.selector, oracle, true
            )
        );
    ...
    function buildListingsRoot(Exchange memory exchange)
        internal
        pure
        returns (bytes32 listingsRoot)
    {
        Exchange[] memory exchanges = new Exchange[](1);
        exchanges[0] = exchange;
        listingsRoot = buildListingsRoot(exchanges);
    }
    ...
    function buildProofArray(Exchange memory exchange, uint256 node)
        internal
        pure
        returns (bytes32[] memory proofArray)
    {
        Exchange[] memory exchanges = new Exchange[](1);
        exchanges[0] = exchange;
        proofArray = buildProofArray(exchanges, node);
    }
    ...
    function getPayload_BuyOfferedERC721WithEther(
        ...
        (TakeAskSingle memory takeAskSingle, bytes memory oracleSignature) =
    ...
    function getPayload_BuyOfferedBETHWithERC721(
        ...
        (TakeBidSingle memory takeBidSingle, bytes memory oracleSignature) =
    ...
    function buildArgumentsDistinctOrders(
        ...
        for (uint256 i; i < nfts.length; ++i) {
            infra.exchanges[i].proof = buildProofArray(infra.exchanges[i], 0);
            infra.orders[i].listingsRoot = buildListingsRoot(infra.exchanges[i]);

            {
                ...
                infra.orderSignature =
                    abi.encodePacked(infra.orderSignature, singleSignature);
            }
        }
